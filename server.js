...
app.post("/subject/people/teacher", function(req, res) {
    ...
    dbConn.query("SELECT Users.user_id,Users.user_name,Users.user_stdid FROM Classroom,Users WHERE Classroom.user_id=Users.user_id AND Classroom.subject_id = ? AND Classroom.user_type_id = 1 OR Classroom.user_type_id = 2", subject_id, function(error, results, fields) {
        if (error) throw error;
        if (results) {
            return res.send(results)
        } else {
